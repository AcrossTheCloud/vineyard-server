Vineyard Mapping Server
=========
### About
vineyard-server is the NodeJS server and web client files for a vineyard mapping web application.
...
### Installation
Download the source code for vineyard-server from github: [https://github.com/AcrossTheCloud/vineyard-server](https://github.com/AcrossTheCloud/vineyard-server) or view the CogniCity installation documentation at [http://cognicity.info](http://cognicity.info).
To check it out using git, run `git clone --recursive git@github.com:AcrossTheCloud/vineyard-server`, which will also check out the default web site submodule [https://github.com/AcrossTheCloud/https://github.com/AcrossTheCloud/cognicity-rem-web](https://github.com/AcrossTheCloud/https://github.com/AcrossTheCloud/cognicity-rem-web), which if you fork you can change to your own set of pages (refer to config.public_dir and config.url_prefix in the config.js file). If you have already cloned the repository, and want to check out the submodule, then run
```shell
git submodule init
git submodule update
```
To update the submodule, first `cd cognicity-rem-web` then `git pull origin master`, then `cd ..` to move back to the main vineyard-server directory and then `git commit` and `git push` along with any other changes.
...
* config.compression - If true, enable Express compression middleware to gzip responses. If deploying standalone then you probably want to set this to true. If deploying behind a reverse proxy+cache like [nginx](http://nginx.org) then you will want to leave this config option set to false and enable compressing using the caching server. For Elastic Beanstalk using nginx this is configured automatically using the Elastic Beanstalk [config file](https://github.com/AcrossTheCloud/vineyard-server/blob/master/.ebextensions/nginx.config) (note if deploying standalone then this file actually contains only a fragment of an nginx config plus the Elastic Beanstalk headers).
* config.logger.logDirectory - the location of the log file for the server. The default is the current working directory, and the default location + name are set up in the [cloud-log-init.config](https://github.com/AcrossTheCloud/vineyard-server/blob/master/.ebextensions/cloud-log-init.config) file for easy export of the log file in Elastic Beanstalk.
...
vineyard-server requires a database that conforms to the [Cognicity framework schema](https://github.com/AcrossTheCloud/cognicity-schema).
